...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [
    ...
    Application.countDocuments({ appliedDate: { $gte: todayStart, $lte: todayEnd }, paymentStatus: "paid" }),
    Payment.aggregate([
      { $match: { status: "paid" } },
      { $group: { _id: null, total: { $sum: "$amount" } } },
    ]),
    Application.countDocuments({
      status: { $in: ["applied", "under_review", "shortlisted", "interview", "pending"] },
      paymentStatus: "paid",
    }),
    Application.countDocuments({ status: "selected", paymentStatus: "paid" }),
    Application.countDocuments({ status: "rejected", paymentStatus: "paid" }),
  ...
  return {
    ...
    inactiveJobs,
    totalUsers,
    totalApplications: paidApplicationsAllTime,
    activeApplications,
    todayApplications,
