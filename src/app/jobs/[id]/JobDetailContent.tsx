...
interface JobDetailContentProps {
  ...
  initialRelated?: JobDetail[];
}

export default function JobDetailContent({ initialJob, initialRelated = [] }: JobDetailContentProps) {
  const params = useParams();
  const router = useRouter();
  const [job, setJob] = useState<JobDetail | null>(initialJob ?? null);
  const [related, setRelated] = useState<JobDetail[]>(initialRelated);
      ...
      <div className="mx-auto max-w-4xl px-4">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <button type="button" onClick={share} className="rounded-lg p-2 text-brand-slate hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Share job">
